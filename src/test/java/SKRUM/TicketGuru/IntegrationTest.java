...
public class IntegrationTest {
	...
	public void setUp() {
		...
		tRepo.save(new Ticket(eRepo.findByName("TestiTapahtuma").get(0),
				ttRepo.findByDescription("Normaali").get(0), trRepo.save(new Transaction(new Date(), 56.34, cRepo.findByName("Testi Pesti").get(0))), "TESTI1", false));
		tRepo.save(new Ticket(eRepo.findByName("TestiTapahtuma2").get(0),
				ttRepo.findByDescription("Opiskelija").get(0), trRepo.save(new Transaction(new Date(), 26.34, cRepo.findByName("Testi Pesti").get(0))), "TESTI2", true));
